...
class ProductTest < ActiveSupport::TestCase
  ...
  test "Product price must be positive" do
    product = Product.new(:title => "My TShirt Title", :description => "yyy", :image_url => "zzz.jpg")
    product.price = -1
    assert product.invalid?
  ...
  def new_product(image_url)
    product.new(:title  => "My TShirt", :description  => "yyy", :unit_price => 1, :image_url  => image_url)
  end
